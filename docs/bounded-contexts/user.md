...
The **User Context** manages individual user identity, profile settings, and localization preferences within the Sancho platform. Unlike tenant-scoped modules, the User context is globally available to an authenticated user regardless of which organization they are currently viewing.
...
- **Account Dashboard**: An overview of the user's roles across all tenants and a summary of their activity.
...
| `/api/user/me/memberships` | `GET` | List all tenants the user belongs to and their associated roles. |
...
4. **Backend Security**: The ASP.NET Core modules use a modern **Secret API Key** to interact securely with Supabase, bypassing RLS only when strictly necessary (e.g., signing avatar URLs or reading cross-tenant roles).
