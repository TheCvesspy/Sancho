...
interface FactionDocumentsPanelProps {
    eventId: string;
    factionId: string;
...
export function FactionDocumentsPanel({ eventId, factionId, initialDocuments, isOrgOrSysAdmin, token }: FactionDocumentsPanelProps) {
    ...
    const handleAdd = async (data: { url: string; displayName: string; documentStatus: "Draft" | "Ready to Review" | "Final" }): Promise<DocumentItem> => {
        ...
        const created = await narrativeApi.addFactionGoogleDriveDocument(token, eventId, factionId, req);
        return created as DocumentItem;
    };

    const handleDelete = async (id: string) => {
        await narrativeApi.deleteFactionDocument(token, eventId, factionId, id);
    };
