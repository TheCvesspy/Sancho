...
interface PlotDocumentsPanelProps {
    eventId: string;
    plotId: string;
...
export function PlotDocumentsPanel({ eventId, plotId, initialDocuments, canWrite, token }: PlotDocumentsPanelProps) {
    ...
    const handleAdd = async (data: { url: string; displayName: string; documentStatus: "Draft" | "Ready to Review" | "Final" }): Promise<DocumentItem> => {
        ...
        const created = await narrativeApi.addPlotGoogleDriveDocument(token, eventId, plotId, req);
        return created as DocumentItem;
    };

    const handleDelete = async (id: string) => {
        await narrativeApi.deletePlotDocument(token, eventId, plotId, id);
    };
