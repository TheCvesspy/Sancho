...
interface PlotlineDocumentsPanelProps {
    eventId: string;
    plotlineId: string;
...
export function PlotlineDocumentsPanel({ eventId, plotlineId, initialDocuments, isOrgOrSysAdmin, token }: PlotlineDocumentsPanelProps) {
    ...
    const handleAdd = async (data: { url: string; displayName: string; documentStatus: "Draft" | "Ready to Review" | "Final" }): Promise<DocumentItem> => {
        ...
        const created = await narrativeApi.addPlotlineGoogleDriveDocument(token, eventId, plotlineId, req);
        return created as DocumentItem;
    };

    const handleDelete = async (id: string) => {
        await narrativeApi.deletePlotlineDocument(token, eventId, plotlineId, id);
    };
