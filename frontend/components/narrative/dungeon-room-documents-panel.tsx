...
interface DungeonRoomDocumentsPanelProps {
    eventId: string;
    locationId: string;
    floorId: string;
    roomId: string;
...
export function DungeonRoomDocumentsPanel({ eventId, locationId, floorId, roomId, canWrite, token }: DungeonRoomDocumentsPanelProps) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const docs = await narrativeApi.listRoomDocumentsNested(token, eventId, locationId, floorId, roomId);
    ...
    }, [token, eventId, locationId, floorId, roomId]);

    const tl: DocumentLinkTranslations = {
        title: t("locations.rooms.documents"),
    ...
    const handleAdd = async (data: { url: string; displayName: string; documentStatus: "Draft" | "Ready to Review" | "Final" }): Promise<DocumentItem> => {
        ...
        const created = await narrativeApi.addRoomGoogleDriveDocumentNested(token, eventId, locationId, floorId, roomId, req);
        return created as DocumentItem;
    };

    const handleDelete = async (id: string) => {
        await narrativeApi.deleteRoomDocumentNested(token, eventId, locationId, floorId, roomId, id);
    };
