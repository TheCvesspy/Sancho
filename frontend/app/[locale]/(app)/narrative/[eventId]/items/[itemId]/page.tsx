...
export default async function ItemDetailPage({
    ...
    params: Promise<{ locale: string; eventId: string; itemId: string }>;
}) {
    const { locale, eventId, itemId } = await params;
    ...
    try {
        const [event, item, assignments, documents] = await Promise.all([
            ...
            narrativeApi.getItem(token, eventId, itemId),
            narrativeApi.listItemAssignments(token, eventId, itemId),
            narrativeApi.listItemDocuments(token, eventId, itemId)
        ]);
        ...
        return (
            <div className="mx-auto w-full max-w-7xl px-6 py-10">
                <div className="bg-destructive/10 border border-destructive/20 text-destructive p-4 rounded-md">
                    <h2 className="font-semibold mb-2">Error loading item. It may have been deleted or you do not have permission to view it.</h2>
                </div>
            </div>
        );
    }
